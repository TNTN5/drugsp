...
<body>
<!-- 搜索框 -->
	<form class="layui-form" action="ordersearch.do">
	  	<div class="layui-form-item search">
		    <div class="layui-input-inline">
		      <input type="search" name="searchu"  lay-verify="required" placeholder="请输入用户id或用户名" autocomplete="off" class="layui-input ">
		    </div>
...
<!-- 表格 -->
  ...
  <c:forEach items="${order}" var="o">
    <tr>
      ...
      <td>
      	<c:forEach items="${shops }" var="s">
      		<c:if test="${o.sid==s.sid }">${s.sname}</c:if>
      	</c:forEach>
      </td>
      <td>
	...
	<tfoot>
	  <tr>
		  <td colspan=9>
		  	<a href="ordersearch.do?searchu=${searchu }&page=${page-1>0?page-1:1} " class="layui-btn">上一页</a>
		  	<input type="text" name="page" value="${page}"/>/
		  	<input type="text" name="page" value="${allPage} " readonly/>
		  	<a href="ordersearch.do?searchu=${searchu }&page=${page+1<allPage?page+1:allPage} " class="layui-btn">下一页</a>
		  </td>
	  </tr>
	  </tfoot>
	</table>
